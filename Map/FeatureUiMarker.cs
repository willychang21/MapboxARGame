	...
	public class FeatureUiMarker : MonoBehaviour
	{
		...
		private void Snap()
		{
			...
			foreach (var vert in _targetVerts)
			{
                ...
                if (Camera.current.tag == "MapCam")
                {
                    var pos = Camera.current.WorldToScreenPoint(_selectedFeature.Transform.position + (_selectedFeature.Transform.lossyScale.x * vert));//加右
            ...
            _landmark.position = new Vector2((right+left)/2 , (top+bottom)/2);
        }
	}
}
